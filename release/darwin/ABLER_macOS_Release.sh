...
if ! "${testing}"; then
    ...
    # bundle.sh 실행
    sh ./bundle.sh --source "${_mount_dir}" --dmg "${_dmg_dir}" --profile abler.carpenstreet.com --codesign "${_codesign_cert}" --sparkle-dir "${SPARKLE_DIR}"
else
    # qt 기본 제공 번들러
    macdeployqt ${_mount_dir}/ABLER.app -verbose=3
    ...
    # bundle.sh 실행
    sh ./bundle.sh --source "${_mount_dir}" --dmg "${_dmg_dir}" --profile abler.carpenstreet.com --codesign "${_codesign_cert}" --sparkle-dir "${SPARKLE_DIR}" --test
fi
