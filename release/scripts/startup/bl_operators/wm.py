...
def context_path_validate(context, data_path):
    try:
        ...
        if str(ex).startswith("'NoneType'"):
            ...
            print(
                "context_path_validate error: context.%s not found (invalid keymap entry?)"
                % data_path
            )
            value = Ellipsis
...
def description_from_data_path(base, data_path, *, prefix, value=Ellipsis):
    ...
    if (rna_prop := context_path_to_rna_property(base, data_path)) and (
...
class WM_OT_context_set_boolean(Operator):
    ...
    def description(cls, context, props):
        return description_from_data_path(
            context, props.data_path, prefix=iface_("Assign"), value=props.value
        )
...
class WM_OT_context_set_int(Operator):  # same as enum
    ...
    def description(cls, context, props):
        return description_from_data_path(
            context, props.data_path, prefix="Assign", value=props.value
        )
...
class WM_OT_context_scale_float(Operator):
    ...
    def description(cls, context, props):
        return description_from_data_path(
            context, props.data_path, prefix=iface_("Scale"), value=props.value
        )
    ...
    def execute(self, context):
        ...
        if getattr(self, "always_step", False):
            if value > 1.0:
                ...
                func = "min"
            exec(
                "context.%s = %s(round(context.%s * value), context.%s + %s)"
...
class WM_OT_context_toggle(Operator):
    ...
    def description(cls, context, props):
        ...
        if props.module:
            return None
        return description_from_data_path(
            context, props.data_path, prefix=iface_("Toggle")
        )
...
class WM_OT_context_toggle_enum(Operator):
    ...
    def description(cls, context, props):
        ...
        return description_from_data_path(
            context, props.data_path, prefix=iface_("Toggle"), value=value
        )
...
class WM_OT_context_cycle_int(Operator):
    ...
    def description(cls, context, props):
        return description_from_data_path(
            context, props.data_path, prefix=iface_("Cycle")
        )
...
class WM_OT_context_menu_enum(Operator):
    ...
    def description(cls, context, props):
        return description_from_data_path(
            context, props.data_path, prefix=iface_("Menu")
        )
    ...
    def execute(self, context):
        ...
        context.window_manager.popup_menu(
            draw_func=draw_cb, title=rna_prop.name, icon=rna_prop.icon
        )
...
class WM_OT_context_pie_enum(Operator):
    ...
    def description(cls, context, props):
        return description_from_data_path(
            context, props.data_path, prefix=iface_("Pie Menu")
        )
    ...
    def invoke(self, context, event):
        ...
        wm.popup_menu_pie(
            draw_func=draw_cb, title=rna_prop.name, icon=rna_prop.icon, event=event
        )
...
class WM_OT_url_open_preset(Operator):
    ...
    preset_items = [
        # Dynamic URL's.
        (
            ("BUG", "Bug", "Report a bug with pre-filled version information"),
            _url_from_bug,
        ),
        (("BUG_ADDON", "Add-on Bug", "Report a bug in an add-on"), _url_from_bug_addon),
        (
            (
                "RELEASE_NOTES",
                "Release Notes",
        ...
        (
            ("MANUAL", "Manual", "The reference manual for this version of Blender"),
            _url_from_manual,
        ),
        # Static URL's.
        (
        ...
        (
            ("BLENDER", "blender.org", "Blender's official web-site"),
            "https://www.blender.org",
        ),
        (
            ("CREDITS", "Credits", "Lists committers to Blender's source code"),
    ...
    if len(id_split) == 1:  # rna, class
        ...
        if operator_exists_pair(class_name, class_prop):
            ...
            if rna_prop:
                ...
                if do_url:
                    url = "%s/bpy.types.bpy_struct.html#bpy.types.bpy_struct.items" % (
                        url_prefix,
                    )
                else:
...
class WM_OT_doc_view_manual(Operator):
    ...
    def _find_reference(rna_id, url_mapping, *, verbose=True):
        ...
        for pattern, url_suffix in url_mapping:
            if fnmatchcase(rna_id, pattern):
                if verbose:
                    print(
                        "            match found: '%s' --> '%s'" % (pattern, url_suffix)
                    )
                return url_suffix
        if verbose:
    ...
    def _lookup_rna_url(rna_id, verbose=True):
        for prefix, url_manual_mapping in bpy.utils.manual_map():
            rna_ref = WM_OT_doc_view_manual._find_reference(
                rna_id, url_manual_mapping, verbose=verbose
            )
    ...
    def execute(self, _context):
        ...
        if url is None:
            self.report(
                ...
                "or callback to bpy.utils.manual_map()" % self.doc_id,
            )
            return {"CANCELLED"}
        else:
            import webbrowser
...
class WM_OT_doc_view(Operator):
    ...
    def execute(self, _context):
        url = _wm_doc_get_id(
            self.doc_id, do_url=True, url_prefix=self._prefix, report=self.report
        )
        if url is None:
            return {"CANCELLED"}
...
class WM_OT_properties_edit(Operator):
    ...
    eval_string: StringProperty(
        name="Value", description="Python value for unsupported custom property types"
    )
    ...
    def _fill_old_ui_data(self, item, name):
        ...
        if self.property_type in {"FLOAT", "FLOAT_ARRAY"}:
            ...
            self.use_soft_limits = (
                self.min_float != self.soft_min_float
                or self.max_float != self.soft_max_float
            )
            default = self._convert_new_value_array(rna_data["default"], float, 32)
            self.default_float = (
                default if isinstance(default, list) else [default] * 32
            )
            ...
            self.use_soft_limits = (
                self.min_int != self.soft_min_int or self.max_int != self.soft_max_int
            )
            self.default_int = self._convert_new_value_array(
                rna_data["default"], int, 32
            )
        ...
        self.is_overridable_library = bool(
            item.is_property_overridable_library('["%s"]' % escaped_name)
        )
    ...
    def _get_converted_value(self, item, name_old, prop_type_new):
        ...
        if prop_type_new == "INT_ARRAY":
            ...
            if prop_type_old in {"INT", "FLOAT", "INT_ARRAY", "FLOAT_ARRAY"}:
                return self._convert_new_value_array(
                    item[name_old], int, self.array_length
                )
        ...
        if prop_type_new == "FLOAT_ARRAY":
            ...
            if prop_type_old in {"INT", "FLOAT", "FLOAT_ARRAY", "INT_ARRAY"}:
                return self._convert_new_value_array(
                    item[name_old], float, self.array_length
                )
    ...
    def _convert_old_ui_data_to_new_type(self, prop_type_old, prop_type_new):
        if prop_type_new in {"INT", "INT_ARRAY"} and prop_type_old in {
            "FLOAT",
            "FLOAT_ARRAY",
        }:
            ...
            self.default_int = self._convert_new_value_array(
                self.default_float, int, 32
            )
        elif prop_type_new in {"FLOAT", "FLOAT_ARRAY"} and prop_type_old in {
            "INT",
            "INT_ARRAY",
        }:
            ...
            self.default_float = self._convert_new_value_array(
                self.default_int, float, 32
            )
    ...
    def _create_ui_data_for_new_prop(self, item, name, prop_type_new):
        if prop_type_new in {"INT", "INT_ARRAY"}:
            ...
            ui_data.update(
                ...
                step=self.step_int,
                default=self.default_int[0]
                if prop_type_new == "INT"
                else self.default_int[: self.array_length],
            ...
            ui_data.update(
                min=self.min_float,
                max=self.max_float,
                soft_min=self.soft_min_float
                if self.use_soft_limits
                else self.min_float,
                soft_max=self.soft_max_float
                if self.use_soft_limits
                else self.max_float,
                step=self.step_float,
                precision=self.precision,
                default=self.default_float[0]
                if prop_type_new == "FLOAT"
                else self.default_float[: self.array_length],
                description=self.description,
        ...
        escaped_name = bpy.utils.escape_identifier(name)
        item.property_overridable_library_set(
            '["%s"]' % escaped_name, self.is_overridable_library
        )

    def _update_blender_for_prop_change(
        self, context, item, name, prop_type_old, prop_type_new
    ):
        from rna_prop_ui import (
    ...
    def execute(self, context):
        ...
        if (
            item.id_data
            and item.id_data.override_library
            and item.id_data.override_library.reference
        ):
        ...
        self._update_blender_for_prop_change(
            context, item, name, prop_type_old, prop_type_new
        )
    ...
    def check(self, context):
        ...
        if self.property_type != self.last_property_type:
            self._convert_old_ui_data_to_new_type(
        ...
        if self.property_type in {"FLOAT", "FLOAT_ARRAY"}:
            ...
            if self.use_soft_limits:
                if self.soft_min_float > self.soft_max_float:
                    self.soft_min_float, self.soft_max_float = (
                        self.soft_max_float,
                        self.soft_min_float,
                    )
                    changed = True
            ...
            if self.use_soft_limits:
                if self.soft_min_int > self.soft_max_int:
                    self.soft_min_int, self.soft_max_int = (
                        self.soft_max_int,
                        self.soft_min_int,
                    )
                    changed = True
...
class WM_OT_properties_edit_value(Operator):
    ...
    def invoke(self, context, _event):
        ...
        if (
            WM_OT_properties_edit.get_property_type(rna_item, self.property_name)
            == "PYTHON"
        ):
    ...
    def draw(self, context):
        ...
        if (
            ...
            col = layout.column(align=True)
            col.prop(
                rna_item, '["%s"]' % escape_identifier(self.property_name), text=""
            )
...
class WM_OT_sysinfo(Operator):
    ...
    def invoke(self, context, _event):
        ...
        if not self.filepath:
            self.filepath = os.path.join(os.path.expanduser("~"), "system-info.txt")
...
class WM_OT_tool_set_by_id(Operator):
    ...
    def execute(self, context):
        ...
        if fn(context, space_type, self.name, as_fallback=self.as_fallback):
            ...
            self.report(
                {"WARNING"}, "Tool %r not found for space %r" % (self.name, space_type)
            )
            return {"CANCELLED"}
...
class WM_OT_toolbar_fallback_pie(Operator):
    ...
    def invoke(self, context, event):
        ...
        def draw_cb(self, context):
            ...
            ToolSelectPanelHelper.draw_fallback_tool_items_for_pie_menu(
                self.layout, context
            )
...
class WM_OT_toolbar_prompt(Operator):
    ...
    def _status_items_generate(cls, keymap, context):
        ...
        status_items.sort(key=lambda a: tool_map_id_to_order[a[0]])
        return status_items
    ...
    def invoke(self, context, event):
        ...
        def status_text_fn(self, context):
            ...
            flow = layout.grid_flow(
                columns=len(status_items), align=True, row_major=True
            )
...
class WM_OT_batch_rename(Operator):
        ...
        if space_type == "SEQUENCE_EDITOR":
            ...
            if data_type == data_type_test:
                data = (
                    ...
                    if only_selected
                    else context.scene.sequence_editor.sequences_all,
                    "name",
                    "Strip(s)",
                )
            ...
            if data_type == data_type_test:
                data = (
                    ...
                    if only_selected
                    else list(space.node_tree.nodes),
                    "name",
                    "Node(s)",
                )
        else:
        ...
        if data is None:
            ...
            if data_type == "OBJECT":
                data = (
                    ...
                    if only_selected
                    else [id for id in bpy.data.objects if id.library is None],
                    "name",
                    "Object(s)",
                )
                ...
                data = (
                    ...
                    if only_selected
                    else [id for id in bpy.data.materials if id.library is None],
                    "name",
                    "Material(s)",
                )
                ...
                data = (
                    ...
                    if only_selected
                    else [id for id in getattr(bpy.data, attr) if id.library is None],
                    "name",
                    descr,
                )
        ...
        for action in actions:
            ...
            if ty == "SET":
                ...
                name = re.sub(
                    ...
                    replace_dst,
                    name,
                    flags=(0 if action.replace_match_case else re.IGNORECASE),
                )
            elif ty == "CASE":
    ...
    def _data_update(self, context):
        ...
        if self._data is None:
            self.data_type = self._data_from_context(
                context, None, False, check_context=True
            )
            self._data = self._data_from_context(context, self.data_type, only_selected)
    ...
    def check(self, context):
        ...
        if (self._data_source_prev != self.data_source) or (
            self._data_type_prev != self.data_type
        ):
        ...
        for action in actions:
            if action.use_replace_regex_src:
                try:
                    ...
                    self.report(
                        {"ERROR"}, "Invalid regular expression (find): " + str(ex)
                    )
                    return {"CANCELLED"}
                ...
                if action.use_replace_regex_dst:
                    try:
                        ...
                        self.report(
                            {"ERROR"},
                            "Invalid regular expression (replace): " + str(ex),
                        )
                        return {"CANCELLED"}
...
class WM_MT_splash_quick_setup(Menu):
    ...
    def draw(self, context):
        ...
        if bpy.types.PREFERENCES_OT_copy_prev.poll(context) and old_version:
            sub.operator(
                "preferences.copy_prev", text="Load %d.%d Settings" % old_version
            )
...
class WM_MT_splash(Menu):
    ...
    def draw(self, context):
        ...
        anchor = col1.operator(
            "acon3d.anchor", text="Don't have an ACON3D account?", icon="USER"
        )
        ...
        col2.operator(
            "wm.url_open_preset",
            text="Blender Release Notes",
            icon="URL",
            text_ctxt="*",
        ).type = "RELEASE_NOTES"
        col2.operator(
            "wm.url_open_preset",
            text="Blender Development Fund",
            icon="FUND",
            text_ctxt="*",
        ).type = "FUND"
...
class WM_MT_splash_about(Menu):
    ...
    def draw(self, context):
        ...
        col.scale_y = 0.8
        col.label(
            text="Built on Blender version " + bpy.app.version_string, translate=False
        )
        col.separator(factor=2.5)
        col.label(
            text=iface_("Date: %s %s")
            % (
        ...
        col.label(
            text=iface_("Hash: %s") % bpy.app.build_hash.decode("ascii"),
            translate=False,
        )
        col.label(
            text=iface_("Branch: %s") % bpy.app.build_branch.decode("utf-8", "replace"),
            translate=False,
        )
        ...
        col.operator(
            "wm.url_open_preset", text="Release Notes", icon="URL"
        ).type = "RELEASE_NOTES"
        ...
        col.operator(
            "wm.url_open_preset", text="Development Fund", icon="FUND"
        ).type = "FUND"
...
class WM_OT_drop_blend_file(Operator):
    ...
    def draw_menu(self, menu, _context):
        ...
        col.operator_context = "INVOKE_DEFAULT"
        col.operator(
            "wm.link", text="Link...", icon="LINK_BLEND"
        ).filepath = self.filepath
        col.operator(
