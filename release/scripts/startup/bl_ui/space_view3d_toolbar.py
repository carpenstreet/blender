...
def is_not_gpencil_edit_mode(context):
    is_gpmode = context.active_object and context.active_object.mode in {
        "EDIT_GPENCIL",
        "PAINT_GPENCIL",
        "SCULPT_GPENCIL",
        "WEIGHT_GPENCIL",
...
class VIEW3D_PT_tools_meshedit_options(View3DPanel, Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        row.active = (
            ob.data.use_mirror_x or ob.data.use_mirror_y or ob.data.use_mirror_z
        )
...
class VIEW3D_PT_tools_brush_color(Panel, View3DPaintPanel):
    ...
    def draw(self, context):
        ...
        draw_color_settings(
            context, layout, brush, color_type=not context.vertex_paint_object
        )
...
class VIEW3D_PT_stencil_projectpaint(View3DPanel, Panel):
    ...
    def draw(self, context):
        ...
        split.column().menu(
            "VIEW3D_MT_tools_projectpaint_stencil", text=stencil_text, translate=False
        )
...
class VIEW3D_PT_tools_brush_stroke_smooth_stroke(
    Panel, View3DPaintPanel, SmoothStrokePanel
):
    bl_context = ".paint_common"  # dot on purpose (access from topbar)
...
class VIEW3D_PT_tools_imagepaint_options_cavity(View3DPaintPanel, Panel):
    ...
    def draw(self, context):
        ...
        layout.template_curve_mapping(
            ipaint, "cavity_curve", brush=True, use_negative_slope=True
        )
...
class VIEW3D_PT_tools_grease_pencil_brush_select(
    ...
    def draw(self, context):
        ...
        row.column().template_ID_preview(
            gpencil_paint, "brush", new="brush.add_gpencil", rows=3, cols=8
        )
        ...
        if context.mode == "PAINT_GPENCIL":
            ...
            if brush is not None:
                col.prop(
                    brush, "use_custom_icon", toggle=True, icon="FILE_IMAGE", text=""
                )
        ...
        if brush is not None:
            ...
            if brush.gpencil_tool in {"DRAW", "FILL"}:
                ...
                if gp_settings.use_material_pin:
                    ...
                    row_mat.template_ID(
                        context.active_object, "active_material", live_icon=True
                    )
                    row_mat.enabled = (
                        False  # will otherwise allow changing material in active slot
                    )
...
class VIEW3D_PT_tools_grease_pencil_brush_random(View3DPanel, Panel):
    ...
    def draw(self, context):
        ...
        row.prop(
            gp_settings, "use_stroke_random_radius", text="", icon="GP_SELECT_STROKES"
        )
        row.prop(
            gp_settings, "use_random_press_radius", text="", icon="STYLUS_PRESSURE"
        )
        ...
        row.prop(
            gp_settings, "use_stroke_random_strength", text="", icon="GP_SELECT_STROKES"
        )
        row.prop(
            gp_settings, "use_random_press_strength", text="", icon="STYLUS_PRESSURE"
        )
        ...
        if gp_settings.use_random_press_uv and self.is_popover is False:
            col.template_curve_mapping(
                gp_settings, "curve_random_uv", brush=True, use_negative_slope=True
            )
        ...
        row.prop(
            gp_settings, "use_stroke_random_hue", text="", icon="GP_SELECT_STROKES"
        )
        ...
        if gp_settings.use_random_press_hue and self.is_popover is False:
            col1.template_curve_mapping(
                gp_settings, "curve_random_hue", brush=True, use_negative_slope=True
            )

        row = col1.row(align=True)
        row.prop(gp_settings, "random_saturation_factor", slider=True)
        row.prop(
            gp_settings, "use_stroke_random_sat", text="", icon="GP_SELECT_STROKES"
        )
        ...
        row.prop(
            gp_settings, "use_stroke_random_val", text="", icon="GP_SELECT_STROKES"
        )
        ...
        if gp_settings.use_random_press_val and self.is_popover is False:
            col1.template_curve_mapping(
                gp_settings, "curve_random_value", brush=True, use_negative_slope=True
            )
        ...
        if gp_settings.use_jitter_pressure and self.is_popover is False:
            col.template_curve_mapping(
                gp_settings, "curve_jitter", brush=True, use_negative_slope=True
            )


class VIEW3D_PT_tools_grease_pencil_brush_paint_falloff(
    GreasePencilBrushFalloff, Panel, View3DPaintPanel
):
    bl_context = ".greasepencil_paint"
    bl_label = "Falloff"
    bl_options = {"DEFAULT_CLOSED"}
...
class VIEW3D_PT_tools_grease_pencil_sculpt_select(
    Panel, View3DPanel, GreasePencilSculptPanel
):
    bl_label = "Brushes"
...
class VIEW3D_PT_tools_grease_pencil_weight_paint_select(
    View3DPanel, Panel, GreasePencilWeightPanel
):
    bl_label = "Brushes"
...
class VIEW3D_PT_tools_grease_pencil_weight_paint_settings(
    Panel, View3DPanel, GreasePencilWeightPanel
):
    bl_label = "Brush Settings"
...
class VIEW3D_PT_tools_grease_pencil_vertex_paint_select(
    View3DPanel, Panel, GreasePencilVertexPanel
):
    bl_label = "Brushes"
...
class VIEW3D_PT_tools_grease_pencil_vertex_paint_settings(
    Panel, View3DPanel, GreasePencilVertexPanel
):
    bl_label = "Brush Settings"
...
class VIEW3D_PT_tools_grease_pencil_brush_vertex_color(View3DPanel, Panel):
    ...
    def poll(cls, context):
        ...
        if context.region.type == "TOOL_HEADER" or brush.gpencil_vertex_tool in {
            "BLUR",
            "AVERAGE",
            "SMEAR",
        }:
...
class VIEW3D_PT_tools_grease_pencil_brush_mixcolor(View3DPanel, Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column()
        col.enabled = (
            settings.color_mode == "VERTEXCOLOR" or brush.gpencil_tool == "TINT"
        )
...
class VIEW3D_PT_tools_grease_pencil_sculpt_options(
    GreasePencilSculptOptionsPanel, Panel, View3DPanel
):
    bl_context = ".greasepencil_sculpt"
...
class VIEW3D_PT_tools_grease_pencil_paint_appearance(
    GreasePencilDisplayPanel, Panel, View3DPanel
):
    bl_context = ".greasepencil_paint"
    bl_parent_id = "VIEW3D_PT_tools_grease_pencil_brush_settings"
    bl_label = "Cursor"
