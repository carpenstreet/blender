...
class TextureNodeCategory(SortedNodeCategory):
    ...
    def poll(cls, context):
        return (
            context.space_data.type == "NODE_EDITOR"
            and context.space_data.tree_type == "TextureNodeTree"
        )
...
def mesh_node_items(context):
    ...
    if geometry_nodes_legacy_poll(context):
        ...
        yield NodeItem(
            "GeometryNodeLegacySubdivisionSurface", poll=geometry_nodes_legacy_poll
        )
        yield NodeItemCustom(draw=lambda self, layout, context: layout.separator())
...
def geometry_node_items(context):
    ...
    if geometry_nodes_legacy_poll(context):
        yield NodeItem(
            "GeometryNodeLegacyDeleteGeometry", poll=geometry_nodes_legacy_poll
        )
...
def point_node_items(context):
    ...
    if geometry_nodes_legacy_poll(context):
        yield NodeItem(
            "GeometryNodeLegacyAlignRotationToVector", poll=geometry_nodes_legacy_poll
        )
        yield NodeItem(
            "GeometryNodeLegacyPointDistribute", poll=geometry_nodes_legacy_poll
        )
        yield NodeItem(
            "GeometryNodeLegacyPointInstance", poll=geometry_nodes_legacy_poll
        )
        yield NodeItem("GeometryNodeLegacyPointScale", poll=geometry_nodes_legacy_poll)
        yield NodeItem(
            "GeometryNodeLegacyPointSeparate", poll=geometry_nodes_legacy_poll
        )
        yield NodeItem(
            "GeometryNodeLegacyPointTranslate", poll=geometry_nodes_legacy_poll
        )
        yield NodeItem(
            "GeometryNodeLegacyRotatePoints", poll=geometry_nodes_legacy_poll
        )
        yield NodeItemCustom(draw=lambda self, layout, context: layout.separator())
    ...
    def contains_group(nodetree, group):
        if nodetree == group:
            ...
            for node in nodetree.nodes:
                if (
                    node.bl_idname in node_tree_group_type.values()
...
def eevee_cycles_shader_nodes_poll(context):
    return cycles_shader_nodes_poll(context) or eevee_shader_nodes_poll(context)
...
def object_eevee_cycles_shader_nodes_poll(context):
    return object_shader_nodes_poll(context) and eevee_cycles_shader_nodes_poll(context)
...
shader_node_categories = [
    ...
    ShaderNodeCategory(
        ...
        items=[
            NodeItem(
                "ShaderNodeOutputMaterial", poll=object_eevee_cycles_shader_nodes_poll
            ),
    ...
    ShaderNodeCategory(
        ...
        items=[
            ...
            NodeItem(
                "ShaderNodeBsdfDiffuse", poll=object_eevee_cycles_shader_nodes_poll
            ),
            NodeItem(
                "ShaderNodeBsdfPrincipled", poll=object_eevee_cycles_shader_nodes_poll
            ),
            NodeItem(
                "ShaderNodeBsdfGlossy", poll=object_eevee_cycles_shader_nodes_poll
            ),
            NodeItem(
                "ShaderNodeBsdfTransparent", poll=object_eevee_cycles_shader_nodes_poll
            ),
            NodeItem(
                "ShaderNodeBsdfRefraction", poll=object_eevee_cycles_shader_nodes_poll
            ),
            NodeItem("ShaderNodeBsdfGlass", poll=object_eevee_cycles_shader_nodes_poll),
            NodeItem(
                "ShaderNodeBsdfTranslucent", poll=object_eevee_cycles_shader_nodes_poll
            ),
            ...
            NodeItem(
                "ShaderNodeBsdfHairPrincipled", poll=object_cycles_shader_nodes_poll
            ),
        ],
    ),
    ShaderNodeCategory(
...
geometry_node_categories = [
    ...
    GeometryNodeCategory(
        ...
        items=[
            NodeItem(
                "GeometryNodeLegacyAttributeRandomize", poll=geometry_nodes_legacy_poll
            ),
            NodeItem(
                "GeometryNodeLegacyAttributeMath", poll=geometry_nodes_legacy_poll
            ),
            NodeItem(
                "GeometryNodeLegacyAttributeClamp", poll=geometry_nodes_legacy_poll
            ),
            NodeItem(
                "GeometryNodeLegacyAttributeCompare", poll=geometry_nodes_legacy_poll
            ),
            NodeItem(
                "GeometryNodeLegacyAttributeConvert", poll=geometry_nodes_legacy_poll
            ),
            NodeItem(
                "GeometryNodeLegacyAttributeCurveMap", poll=geometry_nodes_legacy_poll
            ),
            NodeItem(
                "GeometryNodeLegacyAttributeFill", poll=geometry_nodes_legacy_poll
            ),
            NodeItem("GeometryNodeLegacyAttributeMix", poll=geometry_nodes_legacy_poll),
            NodeItem(
                "GeometryNodeLegacyAttributeProximity", poll=geometry_nodes_legacy_poll
            ),
            NodeItem(
                "GeometryNodeLegacyAttributeColorRamp", poll=geometry_nodes_legacy_poll
            ),
            NodeItem(
                "GeometryNodeLegacyAttributeVectorMath", poll=geometry_nodes_legacy_poll
            ),
            NodeItem(
            ...
            NodeItem(
                "GeometryNodeLegacyAttributeCombineXYZ", poll=geometry_nodes_legacy_poll
            ),
            NodeItem(
            ...
            NodeItem(
                "GeometryNodeLegacyAttributeMapRange", poll=geometry_nodes_legacy_poll
            ),
            NodeItem(
                "GeometryNodeLegacyAttributeTransfer", poll=geometry_nodes_legacy_poll
            ),
    ...
    GeometryNodeCategory(
        "GEO_MATERIAL", "Material", items=geometry_material_node_items
    ),
    ...
    GeometryNodeCategory(
        ...
        items=[
            NodeItem(
                "GeometryNodeLegacyPointsToVolume", poll=geometry_nodes_legacy_poll
            ),
